    ...
    <div x-data="{ openEditModal: false,  openCreateModal: false }">
                        ...
                        <form action="{{ route('oportunidades.index') }}" method="GET">
                            <div class="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 md:space-x-4 mb-6">
                                ...
                                <div class="w-full md:w-1/4">
                                    <select
                                        name="statusFilter"
                                        class="w-full rounded-md border-gray-300 dark:bg-gray-700 dark:border-gray-600 text-gray-900"
                                    >
                                ...
                                <div class="w-full md:w-1/4">
                                    <select
                                        ...
                                        class="w-full rounded-md border-gray-300 dark:bg-gray-700 dark:border-gray-600 text-gray-900"
                                    >
                                        <option value="Filtrar por Responsável" class="text-gray-900">Filtrar por Responsável</option>
                                        @php
                                        ...
                                        @foreach($responsavel as $user)
                                            <option value="{{ $user }}" @selected(request('responsavelFilter') == $user) class="text-gray-900">{{ $user }}</option>
                                        @endforeach
                                    </select>
                                </div>
        ...
        <div x-show="openCreateModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50" style="display: none;">
            <div class="bg-white p-6 rounded shadow-lg w-96">
                ...
                <button @click="openCreateModal = false" class="mt-4 px-4 py-2 bg-gray-600 text-gray-900 rounded">Fechar</button>
            </div>
        </div>
</x-app-layout>